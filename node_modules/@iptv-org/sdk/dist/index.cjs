...
var core = require('@freearhey/core');
var doFetch = require('@ntlab/sfetch');
var sjs = require('@freearhey/search-js');

var store = {
  blocklistRecordsGroupedByChannel: new core.Dictionary(),
  categoriesKeyById: new core.Dictionary(),
  channelsGroupedByCountry: new core.Dictionary(),
  channelsKeyById: new core.Dictionary(),
  citiesKeyByCode: new core.Dictionary(),
  countriesKeyByCode: new core.Dictionary(),
  feedsGroupedByChannel: new core.Dictionary(),
  feedsKeyByStreamId: new core.Dictionary(),
  guidesGroupedByChannel: new core.Dictionary(),
  guidesGroupedByStreamId: new core.Dictionary(),
  languagesKeyByCode: new core.Dictionary(),
  logosGroupedByChannel: new core.Dictionary(),
  logosGroupedByStreamId: new core.Dictionary(),
  regionsKeyByCode: new core.Dictionary(),
  streamsGroupedByChannel: new core.Dictionary(),
  streamsGroupedById: new core.Dictionary(),
  subdivisionsKeyByCode: new core.Dictionary(),
  timezonesKeyById: new core.Dictionary(),
  cities: new core.Collection(),
  regions: new core.Collection(),
  subdivisions: new core.Collection()
};
...
class BroadcastArea {
  ...
  getLocations() {
    return new core.Collection(this.codes).map(
...
class Channel {
  ...
  getCategories() {
    const categories = new core.Collection();
  ...
  getFeeds() {
    return new core.Collection(store.feedsGroupedByChannel.get(this.id));
  }
  ...
  getBroadcastAreaCodes() {
    const broadcastAreaCodes = new core.Collection();
  ...
  getBroadcastAreaLocations() {
    const broadcastAreaLocations = new core.Collection();
    this.getFeeds().forEach((feed) => {
  ...
  getLanguages() {
    const languages = new core.Collection();
    this.getFeeds().forEach((feed) => {
  ...
  getTimezones() {
    const timezones = new core.Collection();
    this.getFeeds().forEach((feed) => {
  ...
  getVideoFormats() {
    const formats = new core.Collection();
    this.getFeeds().forEach((feed) => {
  ...
  getLogos() {
    return new core.Collection(store.logosGroupedByChannel.get(this.id));
  }
  /** @returns List of all available streams for the channel */
  getStreams() {
    return new core.Collection(store.streamsGroupedByChannel.get(this.id));
  }
  /** @returns List of all available guides for the channel */
  getGuides() {
    return new core.Collection(store.guidesGroupedByChannel.get(this.id));
  }
  /** @returns List of records in the blocklist associated with the channel */
  getBlocklistRecords() {
    return new core.Collection(store.blocklistRecordsGroupedByChannel.get(this.id));
  }
  ...
  getSearchable() {
    ...
    const feedAltNames = new core.Collection();
    this.getFeeds().forEach((feed) => {
      feedAltNames.concat(new core.Collection(feed.alt_names));
    });
    feedAltNames.uniq();
    ...
    const broadcastAreaNames = new core.Collection();
...
class Subdivision {
  ...
  getRegions() {
    const country = this.getCountry();
    if (!country) return new core.Collection();
...
class Region {
  ...
  getRegions() {
    return store.regions.filter(
      (region) => !new core.Collection(region.countries).intersects(new core.Collection(this.countries)).isEmpty()
    );
  }
  /** @returns List of country of the region */
  getCountries() {
    const countries = new core.Collection();
  ...
  getSubdivisions() {
    const subdivisions = new core.Collection();
  ...
  getCities() {
    const cities = new core.Collection();
    this.getCountries().forEach((country) => {
...
class Country {
  ...
  getLanguages() {
    const languages = new core.Collection();
    new core.Collection(this.languages).forEach((code) => {
  ...
  getChannels() {
    return new core.Collection(store.channelsGroupedByCountry.get(this.code));
  }
...
class Feed {
  ...
  getStreams() {
    const streams = new core.Collection(store.streamsGroupedById.get(this.getStreamId()));
    if (this.is_main) {
      const otherStreams = new core.Collection(store.streamsGroupedById.get(this.channel));
  ...
  getGuides() {
    const guides = new core.Collection(store.guidesGroupedByStreamId.get(this.getStreamId()));
    if (this.is_main) {
      const otherGuides = new core.Collection(store.guidesGroupedByStreamId.get(this.channel));
  ...
  getBroadcastAreaCodes() {
    return new core.Collection(this.getBroadcastArea().codes);
  }
  ...
  getLogos() {
    return new core.Collection(store.logosGroupedByStreamId.get(this.getStreamId()));
  }
  /** @returns JSON version of all data */
  toJSON() {
    return JSON.stringify(this.toObject());
...
class Stream {
  ...
  getFeed() {
    ...
    if (!this.channel) return;
    if (!this.feed) {
      const channelFeeds = new core.Collection(store.feedsGroupedByChannel.get(this.channel));
  ...
  getBroadcastAreaCodes() {
    ...
    if (feed) return new core.Collection(feed.broadcast_area);
  ...
  getLogos() {
    const channel = this.getChannel();
    if (!channel) return new core.Collection();
...
class DataProcessor {
  process(data) {
    const blocklist = new core.Collection(data.blocklist).map(
      (data2) => new BlocklistRecord(data2)
    );
    const categories = new core.Collection(data.categories).map((data2) => new Category(data2));
    const channels = new core.Collection(data.channels).map((data2) => new Channel(data2));
    const cities = new core.Collection(data.cities).map((data2) => new City(data2));
    const countries = new core.Collection(data.countries).map((data2) => new Country(data2));
    const feeds = new core.Collection(data.feeds).map((data2) => new Feed(data2));
    const guides = new core.Collection(data.guides).map((data2) => new Guide(data2));
    const languages = new core.Collection(data.languages).map((data2) => new Language(data2));
    const logos = new core.Collection(data.logos).map((data2) => new Logo(data2));
    const regions = new core.Collection(data.regions).map((data2) => new Region(data2));
    const streams = new core.Collection(data.streams).map((data2) => new Stream(data2));
    const subdivisions = new core.Collection(data.subdivisions).map(
      (data2) => new Subdivision(data2)
    );
    const timezones = new core.Collection(data.timezones).map((data2) => new Timezone(data2));
...
class DataManager {
  ...
  getProcessedData() {
    return this.#processedData || {
      blocklist: new core.Collection(),
      categories: new core.Collection(),
      channels: new core.Collection(),
      cities: new core.Collection(),
      countries: new core.Collection(),
      feeds: new core.Collection(),
      guides: new core.Collection(),
      languages: new core.Collection(),
      logos: new core.Collection(),
      regions: new core.Collection(),
      streams: new core.Collection(),
      subdivisions: new core.Collection(),
      timezones: new core.Collection()
    };
  }
  getRawData() {
    return this.#rawData;
...
class Client {
  ...
  searchChannels(query) {
    if (!this.#searchIndex || !this.#channelsKeyById) return new core.Collection();
    const results = this.#searchIndex.search(query);
    const channels = new core.Collection();
